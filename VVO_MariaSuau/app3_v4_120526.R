  ...
  setMethod(".renderOutput", "CircosPlot",
            function(x, se, ..., output, pObjects, rObjects) {
              ...
              output[[panel_name]] <- renderPlot({
                ...
                # Forcem reactivitat
                force(rObjects[[paste0(panel_name, "_INTERNAL_rerender")]])
                ...
                bed_data <- data.frame(
                  ...
                  p     = as.numeric(rowData(se)[[padj_nm]])
                )
                
                # Eliminem files amb qualsevol NA a les 6 primeres columnes
                bed_data <- bed_data[complete.cases(bed_data[, 1:6]), ]
                
                # Ens quedem només amb els cromosomes estàndards (autosomes + X + Y)
                bed_data <- bed_data[
                  bed_data$chr %in% paste0("chr", c(1:22, "X", "Y")),
                ]
                
                # Validem que queden dades després dels filtres
                validate(
                  need(nrow(bed_data) > 0,"No valid data to plot")
                )
                
                # Ens quedem amb els que compleixen padj < 0.05 i |logFC| > 1
                bed_data <- bed_data[
                  bed_data$p < 0.05 &
                    (abs(bed_data$val1) > 1 |
                       abs(bed_data$val2) > 1), ]
                ...
                if (nrow(bed_sig) > 0) {
                  # Dibuixem un histograma de densitat genòmica al primer anell
                  # Com més dens = més gens significatius en aquella regió
                ...
                # ------- TRACK 3: logFC del contrast 2 (GEM) ---------
                circlize::circos.genomicTrackPlotRegion(
                  
                  bed_data[, c("chr", "start", "end", "val2")],
                  
                  ylim = range(c(bed_data$val2, 0), na.rm = TRUE),
